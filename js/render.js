...
function render() {
	...
	switch (gameState) {
		case 0:
			...
			ctx.globalAlpha = 1;
			fillText('110px Helvetica', 'Circles', trueCanvas.width/2, trueCanvas.height/2 - 115 * settings.scale);
			fillText('40px Helvetica', 'Jogar!', trueCanvas.width/2, trueCanvas.height/2);
			break;
		...
		case 2:
			if ('initText' in window && initText - 50 > 0) {
				...
				if (op > 1) op = 1;
				ctx.globalAlpha = op;
				fillText('30px Helvetica', settings.mobile ? 'Toque nos lados esquerdo e direito da tela para mover' : 'Use as setas do teclado para mover para a esquerda e para a direita!', trueCanvas.width/2, trueCanvas.height/2 - 170 * settings.scale, 0);
				fillText('30px Helvetica', 'Evite tocar nos blocos (elementares)!', trueCanvas.width/2, trueCanvas.height/2 - 130 * settings.scale, 0);
				ctx.globalAlpha = 1;
				initText--;
			}
		...
		case 3:
			...
			ctx.fillStyle = '#000';
			ctx.fillRect(0, 0, trueCanvas.width, trueCanvas.height);
			fillText('90px Helvetica', 'Fim de jogo', trueCanvas.width/2, trueCanvas.height/2 - 75 * settings.scale);
			fillText('50px Helvetica', 'Pontos: ' + score, trueCanvas.width/2, trueCanvas.height/2);
			fillText('30px Helvetica', settings.mobile ? 'Toque para reiniciar!' : 'Pressione Enter para reiniciar!', trueCanvas.width/2, trueCanvas.height/2 + 60 * settings.scale);
			ctx.globalAlpha = 1;
			break;
	}
}
